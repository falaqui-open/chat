...
function callGet(success, error, opts) 
{
    const language = navigator.userLanguage || navigator.language;
    const record = localeList.find((item) =>{
        const langList = item["Language Codes"].toLowerCase().split(",");
        const found = langList.indexOf(language.toLowerCase()) > -1;
        return found;
    })

    const country = record != null ? record.ISO2 : "";
...
require('cordova/exec/proxy').add('DeviceLocale', module.exports);

// Source: https://gist.github.com/ssskip/5a94bfcd2835bf1dea52?permalink_comment_id=4199253#gistcomment-4199253
const localeList = [
    {
