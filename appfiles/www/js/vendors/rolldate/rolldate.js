...
(function (global, factory) {
    ...
    function Rolldate() {
      ...
      if (config.el) {
        ...
        el.classList.add("no-browser-picker");
    ...
    Rolldate.prototype = {
      ...
      createUI: function createUI() {
        ...
        var contentValue = $(config.el).value;
        var contentValueDate = null;
        if(contentValue.indexOf(":") > -1)
        {
            contentValueDate = new Date(contentValue);
        }
        else
        {
            contentValueDate = new Date(`${contentValue}T00:00`);
        }
        
        var validContent = isNaN(Date.parse(contentValue)) == false;

        var distanceFromBottom = config.distanceFromBottom;

        var domId = data.domId,
            FormatArr = config.format.split(/-|\/|\s|:/g),
            len = FormatArr.length,
            ul = '',
            date = config.el ? (validContent == true ? contentValueDate : ($(config.el).bindDate || new Date())) : (validContent == true ? contentValueDate : _this.bindDate || new Date()), 
            itemClass = '',
            lang = config.lang;
        ...
        var rollPanelCustomStyle = ``;
        if(distanceFromBottom != null)
        {
            rollPanelCustomStyle = `bottom: ${distanceFromBottom}px;`
        }


        if(rollPanelCustomStyle.length > 0)
        {
            rollPanelCustomStyle = `style="${rollPanelCustomStyle}"`;
        }

        var $html = `
        <div class="rolldate-mask">
        </div>
        <div class="rolldate-panel" ${rollPanelCustomStyle}>
            <header>
                ...
                <div class="row no-margin">
                    <div class="col s4 rolldate-calendar-header rolldate-no-padding rolldate-center">${lang.headerYear}</div>
                    <div class="col s4 rolldate-calendar-header rolldate-no-padding rolldate-center">${lang.headerMonth}</div>
                    <div class="col s4 rolldate-calendar-header rolldate-no-padding rolldate-center">${lang.headerDay}</div>
                </div>
            </header>
            <section class="rolldate-content">
                <div class="rolldate-dim mask-top">
                </div>
                <div class="rolldate-dim mask-bottom">
                </div>
                <div class="rolldate-wrapper">
                    ${ul}
                </div>
