...
/**
 * GET /services/miniappsource/:appid
 * Download Miniapp
 * Eg.: http://localhost:24011/services/miniappsource/a123
*/
router.get('/miniappsource/:appid', async (req, res) => {
    try 
    {
        const appid = req.params.appid;
        
        // Assuming a predefined file path for demonstration
        const fileZip = path.resolve(`${__dirname}/../local_modules/miniapp/${appid}.zip`);
        ...
        const fileSize = fs.statSync(fileZip).size;
        ...
        if (range) 
        {
            ...
            const fileStream = fs.createReadStream(fileZip, { start, end });
        ...
        else 
        {
            ...
            res.set({
                ...
                'Content-Disposition': `attachment; filename=miniapp.zip`
            });

            const fileStream = fs.createReadStream(fileZip);
